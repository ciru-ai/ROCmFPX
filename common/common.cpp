...
void common_prompt_checkpoint::update_tgt(
    ...
    const size_t n = llama_state_seq_get_data_ext_storage(
            ctx, data_tgt.data(), ckpt_size, seq_id, flags, storage_tgt);
    if (n != ckpt_size) {
...
void common_prompt_checkpoint::update_dft(
    ...
    const size_t n = llama_state_seq_get_data_ext_storage(
            ctx, data_dft.data(), ckpt_size, seq_id, flags, storage_dft);
    if (n != ckpt_size) {
        GGML_ABORT("checkpoint size mismatch: expected %zu, got %zu\n", ckpt_size, n);
    }
}
...
bool common_prompt_checkpoint::load_tgt(
    ...
    const size_t n = llama_state_seq_set_data_ext_storage(
            ctx, data_tgt.data(), data_tgt.size(), seq_id, flags, storage_tgt);
    if (n != data_tgt.size()) {
...
bool common_prompt_checkpoint::load_dft(
    ...
    const size_t n = llama_state_seq_set_data_ext_storage(
            ctx, data_dft.data(), data_dft.size(), seq_id, flags, storage_dft);
    if (n != data_dft.size()) {
