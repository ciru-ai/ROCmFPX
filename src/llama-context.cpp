...
public:
    ...
    bool finalize() {
        ...
        if (buf_size != 0) {
            LLAMA_LOG_WARN("%s: device checkpoint restore left %zu unread bytes; refusing restore\n", __func__, buf_size);
            valid = false;
            return false;
        }
...
size_t llama_context::state_seq_set_data(llama_seq_id seq_id, const uint8_t * src, size_t size, llama_state_seq_flags flags, const llama_memory_buffers * storage) {
    ...
    if (flags & LLAMA_STATE_SEQ_FLAGS_ON_DEVICE) {
        ...
        auto io_dev_owner = std::make_unique<llama_io_read_device>(src, size, *storage_cur);
        io_dev = io_dev_owner.get();
        io = std::move(io_dev_owner);
    } else {
        io = std::make_unique<llama_io_read_host>(src, size);
    }
