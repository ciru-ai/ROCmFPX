...
public:
    ...
    bool finalize() {
        ...
        for (auto & [buft, mbuf] : mbufs_new) {
            ...
            for (size_t i = 0; i < mbuf_cur.cpy.size(); ++i) {
                ggml_backend_tensor_copy(mbuf_cur.cpy[i], mbuf.org[i]);
            }
        }

        if (buf_size != 0) {
            LLAMA_LOG_WARN("%s: device checkpoint restore left %zu unread metadata bytes; refusing restore\n",
                    __func__, buf_size);
            valid = false;
            return false;
        }
...
static llama_memory_buffers llama_memory_buffers_clone(const llama_memory_buffers & src) {
    ...
    for (const auto & [buft, mbuf_src] : src) {
        ...
        for (auto * tensor : mbuf_src.cpy) {
            mbuf_dst.cpy.push_back(ggml_new_tensor_1d(
                    mbuf_dst.ctx.get(), tensor->type, ggml_nelements(tensor)));
        }
...
llama_state_seq_storage * llama_state_seq_storage_clone(const llama_state_seq_storage * storage) {
    ...
    auto result = std::make_unique<llama_state_seq_storage>();
    result->buffers = llama_memory_buffers_clone(storage->buffers);
