...
void llama_model_saver::add_kv_from_model() {
    ...
    const bool clamp_per_layer = model->arch == LLM_ARCH_BAILINGMOE3;
    add_kv(LLM_KV_SWIGLU_CLAMP_EXP,                  hparams.swiglu_clamp_exp,   clamp_per_layer);
    add_kv(LLM_KV_SWIGLU_CLAMP_SHEXP,                hparams.swiglu_clamp_shexp, clamp_per_layer);
