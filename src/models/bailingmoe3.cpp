...
void llama_model_bailingmoe3::load_arch_hparams(llama_model_loader & ml) {
    ...
    // MTP layers are MLA (full attention) and must be flagged non-recurrent.
    {
        ...
        for (uint32_t i = 0; i < hparams.n_layer_all; ++i) {
            hparams.is_recr_impl[i] = (i < n_main) && hparams.n_head_kv(i) == 0;
        }
    }
...
llama_model_bailingmoe3::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    GGML_ASSERT(hparams.n_layer_nextn > 0 && "BAILINGMOE3 MTP requires n_layer_nextn > 0");
    GGML_ASSERT(hparams.n_layer_nextn == 1 && "BAILINGMOE3 MTP currently only supports a single MTP block");

    const int il = (int) hparams.n_layer() + cparams.nextn_layer_offset;
    GGML_ASSERT(cparams.nextn_layer_offset >= 0 &&
                cparams.nextn_layer_offset < (int) hparams.n_layer_nextn &&
                "nextn_layer_offset out of range [0, n_layer_nextn)");
    const auto & layer = model.layers[il];

    GGML_ASSERT(!hparams.is_recr(il) && "BAILINGMOE3 MTP block must be a full-attention (MLA) layer");
